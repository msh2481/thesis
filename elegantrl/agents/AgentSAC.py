...
class AgentSAC(AgentBase):
    def __init__(
        ...
        self.num_ensembles = getattr(
            args, "num_ensembles", 4
...
class AgentModSAC(
    ...
    def update_objectives(
        ...
        self.update_a = (
            0 if update_t == 0 else self.update_a
        )  # reset update_a to 0 when update_t is 0
        if (self.update_a / (update_t + 1)) < (
            1 / (2 - reliable_lambda)
        ):  # auto Two-time update rule
            self.update_a += 1
...
class ActorSAC(ActorBase):
    def __init__(self, net_dims: List[int], state_dim: int, action_dim: int):
        ...
        self.net_s = build_mlp(
            dims=[state_dim, *net_dims], if_raw_out=False
        )  # network of encoded state
        self.net_a = build_mlp(
            dims=[net_dims[-1], action_dim * 2]
    ...
    def get_action_logprob(self, state):
        ...
        logprob -= (
            -action_tanh.pow(2) + 1.000001
        ).log()  # fix logprob using the derivative of action.tanh()
...
class ActorFixSAC(ActorBase):
    def __init__(self, net_dims: List[int], state_dim: int, action_dim: int):
        ...
        self.decoder_a_avg = build_mlp(
            dims=[net_dims[-1], action_dim]
        )  # decoder of action mean
        self.decoder_a_std = build_mlp(
            dims=[net_dims[-1], action_dim]
        )  # decoder of action log_std
    ...
    def get_action_logprob(self, state: TEN) -> Tuple[TEN, TEN]:
        ...
        logprob -= (
            np.log(2.0) - action - self.soft_plus(-2.0 * action)
...
class CriticEnsemble(CriticBase):
    def __init__(
        ...
        self.encoder_sa = build_mlp(
            dims=[state_dim + action_dim, net_dims[0]]
    ...
    def get_q_values(self, state: TEN, action: TEN) -> TEN:
        ...
        values = th.concat(
            [decoder_q(tensor_sa) for decoder_q in self.decoder_qs], dim=-1
        )
