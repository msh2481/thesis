...
class Config:
    def __init__(self, agent_class=None, env_class=None, env_args=None):
        ...
        self.state_value_tau = (
            0  # the tau of normalize for value and state `std = (1-std)*std + tau*std`
        )
        ...
        if self.if_off_policy:  # off-policy
            ...
            self.lambda_fit_cum_r = (
                0.0  # critic fits the mean of a batch cumulative rewards
            )
            self.buffer_init_size = int(
                self.batch_size * 8
            ...
            self.buffer_init_size = (
                None  # train after samples over buffer_init_size for off-policy
            )
        ...
        self.num_threads = (
            8  # cpu_num for pytorch, `th.set_num_threads(self.num_threads)`
        )
        ...
        self.break_score = (
            np.inf
        )  # break training if `cumulative_rewards > break_score`
        self.if_keep_save = (
        ...
        self.eval_record_step = (
            0  # evaluator start recording after the exploration reaches this step.
        )
...
def build_env(env_class=None, env_args: dict = None, gpu_id: int = -1):
    ...
    if env_args.get("if_build_vec_env"):
        ...
        env = VecEnv(
            env_class=env_class, env_args=env_args, num_envs=num_envs, gpu_id=gpu_id
        )
    elif env_class.__module__ == "gymnasium.envs.registration":
        env = env_class(
            id=env_args["env_name"], render_mode=env_args.get("render_mode", "human")
        )
    else:
...
def kwargs_filter(function, kwargs: dict) -> dict:
    ...
    logger.info(
        f"common args: {common_args}. filtered out: {kwargs.keys() - common_args}"
    )
...
def get_gym_env_args(env, if_print: bool) -> dict:
    ...
    if if_gym_standard_env and (
        not hasattr(env, "num_envs")
        ...
        if if_discrete:  # make sure it is discrete action space
            ...
            str(env.action_space).find("Box") >= 0
        ):  # make sure it is continuous action space
            action_dim = env.action_space.shape[0]
            if any(env.action_space.high - 1):
                print(
                    f"| WARNING: env.action_space.high {env.action_space.high}",
                    flush=True,
                )
            if any(env.action_space.low + 1):
                print(
                    f"| WARNING: env.action_space.low {env.action_space.low}",
                    flush=True,
                )
        else:
            raise RuntimeError(
...
class SubEnv(Process):
    ...
    def run(self):
        ...
        """build env"""
        if (
            self.env_class.__module__ == "gymnasium.envs.registration"
        ):  # is standard OpenAI Gym env
...
class VecEnv:
    def __init__(
        self, env_class: object, env_args: dict, num_envs: int, gpu_id: int = -1
    ):
    ...
    def step(
        self, action: TEN
    ) -> Tuple[TEN, TEN, TEN, TEN, dict]:  # agent interacts in env
...
def check_vec_env():
    ...
    env_class = (
        gym.make
    )  # run a custom env: PendulumEnv, which based on OpenAI pendulum
    env_args = {
    ...
    for i in range(4):
        ...
        print(
            f"| num_envs {num_envs}  {[t.shape for t in (state, reward, terminal, truncate)]}",
            flush=True,
        )
