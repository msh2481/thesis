...
class Evaluator:
    ...
    def evaluate_and_save(
        self, actor: th.nn, steps: int, exp_r: float, logging_tuple: tuple
    ):
        ...
        if self.tensorboard:
            self.tensorboard.add_scalar(
                "info/critic_loss_sample", logging_tuple[0], self.total_step
            )
            self.tensorboard.add_scalar(
                "info/actor_obj_sample", -1 * logging_tuple[1], self.total_step
            )
            self.tensorboard.add_scalar(
            ...
            self.tensorboard.add_scalar(
                "info/critic_loss_time", logging_tuple[0], train_time
            )
            self.tensorboard.add_scalar(
                "info/actor_obj_time", -1 * logging_tuple[1], train_time
            )
        ...
        if if_save:  # save checkpoint with the highest episode return
            if self.if_over_write:
                ...
                actor_path = (
                    f"{self.cwd}/actor__{self.total_step:012}_{self.max_r:09.3f}.pt"
                )
    ...
    def get_cumulative_rewards_and_step_single_env(self, actor) -> TEN:
        rewards_steps_list = [
            get_rewards_and_steps(self.env, actor) for _ in range(self.eval_times)
        ]
    ...
    def save_training_curve_jpg(self):
        ...
        fig_title = (
            f"step_time_maxR_{int(total_step)}_{int(train_time)}_{self.max_r:.3f}"
        )
...
def demo_evaluator_actor_pth():
    ...
    from elegantrl.train.config import build_env, Config
