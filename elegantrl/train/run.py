...
from .config import build_env, Config
from .evaluator import Evaluator, get_rewards_and_steps
...
def train_agent(args: Config, if_single_process: bool = False):
    if if_single_process:
        ...
        print(
            f"| train_agent_multiprocessing_multi_gpu() with GPU_ID {args.learner_gpu_ids}",
            flush=True,
        )
...
def train_agent_multiprocessing(args: Config):
    ...
    method = (
        "spawn" if os.name == "nt" else "forkserver"
    )  # os.name == 'nt' means Windows NT operating system (WinOS)
    mp.set_start_method(method=method, force=True)

    """build the Pipe"""
    worker_pipes = [
        Pipe(duplex=False) for _ in range(args.num_workers)
    ]  # receive, send
...
class Learner(Process):
    ...
    def run(self):
        ...
        learner_id = (
            args.learner_gpu_ids.index(args.gpu_id)
            if len(args.learner_gpu_ids) > 0
            else 0
        )
        ...
        states = th.zeros(
            (horizon_len, num_seqs, state_dim), dtype=th.float32, device=agent.device
        )
        actions = (
            th.zeros(
            ...
            if not if_discrete
            else th.zeros((horizon_len, num_seqs, 1), dtype=th.int32).to(agent.device)
        )
        rewards = th.zeros(
            (horizon_len, num_seqs), dtype=th.float32, device=agent.device
        )
        ...
        while if_train:
            actor = agent.act
            actor = (
                deepcopy(actor).cpu() if os.name == "nt" else actor
            ...
            for shift_id in range(num_communications):
                ...
                for buffer_item, buffer_tensor in zip(
                    _buffer_items_tensor, buffer_items_tensor
                ):
            ...
            if (
                self.eval_pipe.poll()
            ):  # whether there is any data available to be read of this pipe0
                if_train = (
                    self.eval_pipe.recv()
...
class EvaluatorProc(Process):
    ...
    def run(self):
        ...
        eval_env = build_env(eval_env_class, eval_env_args, args.gpu_id)
        evaluator = Evaluator(
            cwd=args.cwd, env=eval_env, args=args, if_tensorboard=False
        )
        ...
        device = th.device(
            f"cuda:{args.gpu_id}"
            if (th.cuda.is_available() and (args.gpu_id >= 0))
            else "cpu"
        )
        del args
        ...
        while if_train:
            ...
            if actor is None:
                ...
                actor = (
                    actor.to(device) if os.name == "nt" else actor
                )  # WindowsNT_OS can only send cpu_tensor
        ...
        while (
            ...
            while self.pipe0.poll():
                try:
                    ...
                    print(
                        "| Evaluator Ignore RuntimeError in self.pipe0.recv()",
                        flush=True,
                    )
                time.sleep(1)
            time.sleep(1)
...
def valid_agent(
    ...
    # )
    actor = th.load(actor_path, weights_only=False)
